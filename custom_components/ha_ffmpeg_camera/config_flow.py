...
def build_schema(
    ...
    spec = {
        ...
        vol.Optional(
            CONF_EXTRA_ARGUMENTS,
            description={"suggested_value": user_input.get(CONF_EXTRA_ARGUMENTS, "")},
        ): str,
    }
    if is_options_flow:
        spec[
...
class FFmpegCamConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user_confirm_still(
        ...
        preview_url = f"/api/ha_ffmpeg_camera/preview_flow_image/{self.flow_id}?t={datetime.now().isoformat()}"
        return self.async_show_form(
...
class CameraImagePreview(HomeAssistantView):
    ...
    url = "/api/ha_ffmpeg_camera/preview_flow_image/{flow_id}"
    name = "api:ha_ffmpeg_camera:preview_flow_image"
    requires_auth = False
    ...
    async def get(self, request: web.Request, flow_id: str) -> web.Response:
        ...
        camera = FFmpegCamera(self.hass, user_input, flow_id, "preview")
        if not camera.is_on:
